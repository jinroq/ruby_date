...
class RubyDateDateAttrTest < Test::Unit::TestCase
  ...
  def test__attr
    ...
    assert_equal('1965-05-23', date.to_s)

    assert_equal('', date.inspect.gsub!(/./,''))
    assert_equal('', date.to_s.gsub!(/./,''))

    assert_equal(2438904, date.jd)

    assert_equal(0, date.day_fraction)

    assert_equal(38903, date.mjd)
    assert_equal(139744, date.ld)

    assert_equal(1965, date.year)
    assert_equal(143, date.yday)
    assert_equal(5, date.mon)
    assert_equal(date.mon, date.month)
    assert_equal(23, date.mday)
    assert_equal(date.mday, date.day)

    assert_equal(false, date.respond_to?(:hour))
    assert_equal(false, date.respond_to?(:min))
    assert_equal(false, date.respond_to?(:sec))
    assert_equal(false, date.respond_to?(:sec_fraction))
    assert_equal(false, date.respond_to?(:zone))
    assert_equal(false, date.respond_to?(:offset))

    assert_equal(1965, date.cwyear)
    assert_equal(20, date.cweek)
    assert_equal(7, date.cwday)

    assert_equal(0, date.wday)
    assert_equal(false, date.leap?)
    assert_equal(false, date.julian?)
    assert_equal(true, date.gregorian?)

    assert_equal(RubyDate::ITALY, date.start)
    assert_equal(date.start, date.start)
